...
- **0.1.10** — Bug fixes: corrected the terrain-RGB encoding (previously decoded ~10x too small), made the hover elevation read the absolute DEM height directly from the COG via a new `/api/viewshed/elevation` endpoint (always correct after moving, unlike MapLibre's cache-dependent terrain read), busted stale cached terrain tiles (which also caused `Invalid LngLat (NaN, NaN)` errors), and converted the interactive overlays (directional cone, search polygon, draft, scored result points) from Deck.gl to **native MapLibre vector layers** so they stay glued to the 3D terrain with no parallax and never vanish while zooming/rotating.
