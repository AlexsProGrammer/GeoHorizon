...
- **0.1.10** — Bug fixes: corrected the terrain-RGB encoding (previously decoded ~10x too small), made the hover elevation read the absolute DEM height directly from the COG via a new `/api/viewshed/elevation` endpoint (always correct after moving), busted stale cached terrain tiles (which also caused `Invalid LngLat (NaN, NaN)` errors), converted the interactive overlays to **native MapLibre vector layers** for no-parallax terrain placement, and fixed map/hover lag — the Deck.gl overlay is now created lazily (only when a viewshed result exists) and the expensive hover queries are skipped while the camera moves.
