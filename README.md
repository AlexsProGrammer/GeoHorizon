...
- **0.1.10** — Bug fixes: corrected the terrain-RGB encoding (previously decoded ~10x too small), made the hover elevation read the absolute DEM height directly from the COG via a new `/api/viewshed/elevation` endpoint (so it's always correct after moving, unlike MapLibre's cache-dependent terrain read), busted stale cached terrain tiles (which also caused `Invalid LngLat (NaN, NaN)` errors), restored Deck.gl interleaved rendering so overlays stay glued to the terrain, and disabled depth-testing on overlays so they never vanish while zooming.
